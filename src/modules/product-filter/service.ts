    ...
    return `
                ...
                ${
                  collectionIds?.length
                    ? `AND (p.collection_id = ANY(ARRAY[${collectionIds
                        .map((id) => `'${id}'`)
                        .join(",")}]))`
                    : ""
                }
                ${
                  categoryIds?.length
                    ? `AND (pcp.product_category_id = ANY(ARRAY[${categoryIds
                        .map((id) => `'${id}'`)
                        .join(",")}]))`
                    : ""
                }
                ...
                ${
                  isDiscountAvailable
                    ? "AND (price->>'is_discount_available')::boolean = true"
                    : ""
                }
                ${
                  brandIds?.length
        ...
        sharedContext.manager.execute(`
                        ...
                        SELECT 
                            ...
                            jsonb_agg(DISTINCT jsonb_build_object(
                             ...
                             )) FILTER (WHERE pc.id IS NOT NULL) as category,
                            jsonb_agg(DISTINCT jsonb_build_object( 
                                'id', img.id,
                                'url', img.url,
                            ...
                            ${avilableStock ? "END" : ""}
                              ...
                              avilableStock
                                ? "FILTER (WHERE vi.total_available_inventory > 0)"
                                : ""
                            } as variants
                        FROM 
                            ${this.getCommonJoins()}
