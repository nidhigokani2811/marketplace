...
export const transformProduct = (product) => ({
    ...
    price: product.variants?.[0]?.prices?.[0]?.amount ?? 0, // Example: Take the first variant's price
...
class MeiliSearchService extends SearchUtils.AbstractSearchService {
    ...
    async addDocuments(indexName: string, documents: any, type: string) {
        ...
        return await this.client_.index(indexName).addDocuments(transformedDocuments);
    }
